...
type PromotionIncentiveActivityRouteContext = {
  params: Promise<{
    id: string;
  }>;
};

export async function GET(request: Request, contextParams: PromotionIncentiveActivityRouteContext) {
  ...
  try {
    ...
    console.error("[h5-bff-route-error]", {
      activityId: id,
      message: error instanceof Error ? error.message : "Promotion incentive activity detail BFF request failed.",
      requestId,
      route: "/api/bff/promotion/activities/[id]"
    });
