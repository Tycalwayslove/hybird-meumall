...
export function PromotionRankingScreen({ data }: { data: RankingData }) {
  ...
  return (
    <AppScreen contentClassName="mx-auto min-h-screen w-full max-w-[430px] overflow-hidden pb-[92px]">
        <section className="relative min-h-[294px] overflow-hidden px-4 pt-[calc(env(safe-area-inset-top)+52px)] text-text-inverse" style={{ background: rankingTheme.heroBackground }}>
        ...
        <section className="-mt-5 px-4">
          {data.rows.length === 0 ? (
            ...
            <div className="overflow-hidden rounded-sheet bg-fill-white shadow-card">
              {rest.map((row) => (
        ...
        <section className="fixed inset-x-0 bottom-0 z-20 px-4 pb-[calc(env(safe-area-inset-bottom)+12px)]">
          <div className="mx-auto flex max-w-[398px] items-center gap-3 rounded-[14px] bg-text-primary px-4 py-3 text-text-inverse shadow-floating">
