...
| `npm run ai:register-release` | 生成 release 注册草案，或在 `--execute` 时将 candidate release 注册到 Java H5 版本管理。 |
...
`.github/workflows/h5-release.yml` 提供手动触发的 H5 SSR 发布流水线。流水线运行质量门禁、`pnpm build`、release 草案生成、standalone 静态资源准备、SSR 发布计划生成，并上传 `.next/standalone`、`.next/static`、`public` 与 `archives/releases/<version>` 作为发布归档。当 `register_release=true` 且配置了 Java H5 版本管理地址时，流水线会调用 `ai:register-release --execute` 将 candidate release 注册到 Java H5 版本管理。active、灰度和回滚由外部 Java 版本管理或受保护发布环境审批后执行。
