...
## ADR-0025 - 收货地址模块先完成页面与真实 BFF 闭环
...
商品详情和订单确认已经接入普通商品快递链路，但订单确认页此前只有静态地址卡，缺少地址列表、新增地址和编辑地址页面。旧 uni-app 项目中已有 `delivery-address` 和 `edit-address` 页面，可迁移主要页面结构和静态资源。

### 决策

本期先在 H5 完成地址模块页面与真实 BFF 闭环：

- 新增 `/address` 地址列表页，支持管理态和订单确认选择态。
- 新增 `/address/edit` 新增/编辑地址页。
- 地址空态图和定位图标复制旧项目资源，并注册到本地资源 registry。
- 订单确认页地址卡跳转 `/address?select=1`。
- 订单确认和订单提交链路支持传递 `addressId/addrId`。
- 订单确认和提交 BFF 已使用 Java `/p/address/addrInfo/{addrId}` 解析默认/选中地址；无地址时禁止提交。
- 地址管理页列表、详情、新增、编辑、删除和设默认通过 `/api/bff/address/*` 接入旧 Java `/p/address/*`。
- 新增/编辑地址页省市区只消费 `/api/bff/address/regions` -> Java `/p/area/listByPid`，接口未返回时不展示本地选项。
- 定位按钮预留 `address.chooseLocation` Bridge，当前只输出 `[MeuMall][address-location]` 调试日志并提示等待 App 接入；App 后续提供真实定位/地图选点。

### 影响

- 订单确认链路具备地址选择页面，不再只有静态地址卡。
- 无默认地址用户可以进入新增地址页保存到 Java 地址接口，再回到地址列表选择使用。
- 地址页不保留本地轻量业务数据兜底；无 token、Bridge/BFF 失败或区域接口为空时展示空态、错误提示或空选项，避免本地数据与后端发货数据不一致。
- 完整全国行政区树和 App 真实定位仍是后续能力，不在当前 H5 侧伪造。

### 备选方案

- 只做页面 mock：不采用，因为无默认地址用户仍无法完成下单。
- 只在订单确认页内做地址弹层：暂不采用，旧项目已有独立地址管理页，且我的页也需要地址管理入口。
...
由于权益中心、轮播、活动页等客户端组件会在 hydrate 后重新计算图片 URL，`assetUrl()` 读取环境变量时必须使用显式 `process.env.NEXT_PUBLIC_H5_ASSET_BASE_URL`、`process.env.NEXT_PUBLIC_H5_BASE_PATH` 引用，禁止使用 `process.env[key]`。动态读取在 Next 客户端 bundle 中可能无法被稳定内联，导致 SSR 首屏路径正确、客户端切换后却退回裸 `/assets/...`。

### 影响

- 新增页面可以在本地、版本目录、后续 CDN 三种资源来源之间复用同一套代码。
- 新会话只要读取 `AGENTS.md`、本 ADR 或推广页面开发规范，就能恢复静态资源规则。
- 测试需要覆盖 basePath 场景，并检查资源工具没有动态 `process.env[...]` 读取，避免只在本地根路径通过。

### 备选方案

- 在每个组件里手动拼 `basePath + "/assets/..."`：拒绝，因为会造成重复逻辑和遗漏。
- 继续直接写 `/assets/...`：拒绝，因为版本目录和后续 CDN 场景都会失效。
- 在资源工具里用 `process.env[key]` 动态读取：拒绝，因为客户端 bundle 无法可靠内联版本配置。
