...
- H5 BFF HTTP 鉴权基础：原生 Cookie 登录态、Next 服务端读取 token、服务端 backend client 转 Authorization、浏览器端 H5 client 请求 BFF。
- 首页原生传参展示面板：通过 `/api/bff/runtime/context` 展示完整 Cookie 值、`meu_page_config`、URL 参数和环境信息；该面板仅限当前内部调试，正式开放前必须删除或关闭。
...
- API token 刷新、重新登录、真实 base URL 和原生代理策略尚未确定；当前已建立 H5 BFF 转 Authorization 的基础设施，但真实 Java / Python base URL 仍需确认。
