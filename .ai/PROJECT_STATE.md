...
- 真实业务页面。
- 秒杀、拼团、自提、同城、商品详情收藏状态切换、优惠券领取、通联微信支付返回字段、App 正式支付宝/微信 SDK handler 和真实支付验签闭环。
...
- v1.2.0 搜索首页热门词、搜索热榜、搜索结果商品和商品分类页已接真实接口；推广商品列表、推广首页、权益中心、我的页、收藏商品、我的足迹和限时秒杀列表已接真实 BFF；收银台已接真实发起支付和通联支付宝回查，但推广商品分类 ID 来源、商品详情收藏状态、搜索建议、秒杀购买资格、秒杀下单、拼团、自提、同城、通联微信支付返回字段和 App 正式支付 SDK handler 仍需后续补齐。
