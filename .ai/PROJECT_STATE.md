...
- 首页 banner 跳转已按 Apifox main 分支 `/p/app/home/index` 的 `PlatformBannerVO.jumpType` 细化：`1=H5` 尊重 URL/路径，`2=商品详情` 进入 `/product/<id>`，`3=商城活动` 优先尊重后端 URL/路径、纯业务 ID 暂进入 `/promotion/activities?activityId=<id>`，`4=达人激励活动` 进入 `/promotion/activities/<id>`，`5=带货排行榜` 支持销量榜、销售额榜和榜单中心；Tab 根路径切 Tab，其它目标新开 H5 WebView。
