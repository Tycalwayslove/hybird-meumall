...
- 推广模块首批页面：已实现推广首页五档达人主题、活动中心、榜单中心、达人销量榜、达人销售额榜和达人权益中心；推广首页 `/promotion` 已接入真实概览 BFF `/api/bff/promotion/home` -> Java `/p/distribution/home/overview`；活动中心 `/promotion/activities` 已接入真实 BFF `/api/bff/promotion/activities` -> Java `/p/app/distribution/incentive/page`；活动详情 `/promotion/activities/[id]` 已聚合 Java `/p/app/distribution/incentive/detail/{id}` 和 `/p/app/distribution/incentive/reward/detail/{id}`。活动列表、详情、奖励详情失败或 token 缺失不回退本地 mock。
...
- 真实业务页面。
- 秒杀、拼团、自提、同城、商品详情收藏状态切换、优惠券领取、通联微信小程序收银台真机回调、App 正式支付宝/微信 SDK handler 和真实支付验签闭环。
...
- 推广激励活动已接入 APP 侧四个 Java 接口：列表 `/p/app/distribution/incentive/page`、详情 `/p/app/distribution/incentive/detail/{id}`、奖励详情 `/p/app/distribution/incentive/reward/detail/{id}`、领取奖励 `PATCH /p/app/distribution/incentive/reward/receive/{recordId}`。H5 BFF 路由为 `/api/bff/promotion/activities`、`/api/bff/promotion/activities/[id]`、`/api/bff/promotion/activities/[id]/reward`、`/api/bff/promotion/activities/rewards/[recordId]/receive`；实物奖励地址选择完整交互后置。
...
- v1.2.0 搜索首页热门词、搜索热榜、搜索结果商品和商品分类页已接真实接口；推广商品列表、推广首页、权益中心、我的页、收藏商品、我的足迹和限时秒杀列表已接真实 BFF；收银台已接真实发起支付、通联支付宝回查和通联微信小程序收银台 Bridge payload，但推广商品分类 ID 来源、商品详情收藏状态、搜索建议、秒杀购买资格、秒杀下单、拼团、自提、同城、通联微信真机回调和 App 正式支付 SDK handler 仍需后续补齐。
- 个人中心二级页当前仍有优惠券领取/使用等静态 mock 页面或动作；钱包和银行卡管理已接真实接口，但提现、提现记录、账户管理和新增绑卡仍后置；收藏商品和我的足迹已接真实接口并支持取消收藏/删除足迹；我的页顶部概览已接真实 BFF；地址管理已接真实 BFF 和 Bridge 优先层，新增/编辑页已有轻量省市区下拉，但完整区域树和 App 定位/地图选点仍后置。
