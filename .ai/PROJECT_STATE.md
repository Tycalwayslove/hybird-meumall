...
- AI 辅助脚本默认只做本地草案、SSR 发布计划和 smoke，不连接真实外部发布平台。
...
- 发布准备和回滚脚本仅生成或修改本地草案；manifest candidate/active 覆盖必须通过受控发布平台或审批流程执行。
...
- manifest 可由 H5 runtime 拉取并缓存，但最终由 H5 拉取、原生拉取还是二者都拉取尚未确定。
...
- GitHub Actions workflow 已切回 SSR/standalone 产物归档，但尚未在远端仓库环境中验证，仍需配置 `H5_SERVICE_BASE_URL` 等 GitHub Secrets 和受保护环境。

## 下一步建议

1. 确认 Native Bridge namespace、通信方式和首批方法。
2. 配置 SSR 部署平台、`H5_SERVICE_BASE_URL`、受保护环境和 manifest active 发布审批。
3. 确认首批需要内置的原生兜底页。
