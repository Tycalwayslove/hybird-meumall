...
- 真实业务页面。
- 秒杀、拼团、自提、同城、商品详情收藏状态切换、优惠券领取、真正确认付款、支付 Bridge 和订单支付结果真实接口。
...
- v1.2.0 搜索首页热门词、搜索热榜、搜索结果商品和商品分类页已接真实接口；推广商品列表、推广首页、权益中心、我的页、收藏商品、我的足迹和限时秒杀列表已接真实 BFF，但推广商品分类 ID 来源、商品详情收藏状态、搜索建议、秒杀购买资格、秒杀下单、拼团、自提、同城和支付仍需后续补齐 BFF service 和接口契约。
- 个人中心二级页当前仍有钱包流水、优惠券领取/使用等静态 mock 页面或动作；收藏商品和我的足迹已接真实接口并支持取消收藏/删除足迹；我的页顶部概览已接真实 BFF；地址管理已接真实 BFF 和 Bridge 优先层，新增/编辑页已有轻量省市区下拉，但完整区域树和 App 定位/地图选点仍后置。
