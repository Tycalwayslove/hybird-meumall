...
- `paySettlementType=1 + payType=8` 时，`/api/bff/order-pay` 将通联微信 `result=0 + chnlFrontParamInfo` 映射为 `paymentStartCashier` 的 `paymentMode=allinpay-mini-program-bridge` payload，并把解析后的 `chnlFrontParamInfo` 对象传给原生。
- `/api/bff/allinpay-order-status` 回查通联支付状态。
- `/pay-way` 调用 `rpc/paymentStartCashier` / `rpc/payment.openUrl` 并进入 `/pay-result`。
...
- `rpc/paymentStartCashier` 需要 iOS / Android 接入正式支付宝/微信 SDK handler，以及 `paymentMode=allinpay-mini-program-bridge` 的喵呜小程序支付桥拉起逻辑。
