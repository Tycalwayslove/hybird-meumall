...
本项目属于 MeuMall 多项目工作区。处理正式业务、跨项目协作、接口契约、发布或长期维护任务时，必须先读取根目录 `.ai-workspace/README.md`，并遵守其中的统一工作项结构、状态流转、验收标准、记忆协议和跨项目契约治理。

子项目规则用于补充 H5 专属约束；当根级规则和本文件都适用时，同时满足两者。若出现冲突，先停止并向用户说明冲突点。
